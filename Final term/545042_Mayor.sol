...
contract Mayor {
    ...
    modifier ballotNotCompleted() {
        require(!ballot_completed, "The ballot has been already completed");
        _;
    }
    ...
    // Indicates if the ballot (the mayor_or_sayonara() function) has been already executed.
    bool ballot_completed;
    ...
    function open_envelope(uint _sigil, bool _doblon) canOpen public payable{
		...
		require(souls[msg.sender].voted == false, "The sender has already opened its envelope and voted");
		...
		_doblon ? yaySoul += msg.value : naySoul += msg.value;
		
		souls[msg.sender].soul = msg.value;
		souls[msg.sender].doblon = _doblon;
		souls[msg.sender].voted = true;
    ...
    function mayor_or_sayonara() canCheckOutcome ballotNotCompleted public {
        ballot_completed = true; // The function can be executed only once
        
		bool winning_vote = yaySoul > naySoul; 
		
		// Each "losing" voter is refunded 
		for (uint i = 0; i < voters.length; i++)
			if ( souls[voters[i]].doblon != winning_vote ) 
				payable(voters[i]).transfer(souls[voters[i]].soul);
		
		
		if (winning_vote) {     // The mayor is confirmed
			candidate.transfer(yaySoul);
			emit NewMayor(candidate);
		} else {    // The mayor has lost
